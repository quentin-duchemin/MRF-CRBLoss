...
class model(BaseModel):
	def __init__(self, nb_params=None, projection=None, ghost=False):
		...
		if not self.ghost:
			...
			#
			# # projection layer for the real part
			# if self.projection.initialization != 'Fixlayer':
			...
			self.fc21 = nn.Linear(2 * self.projection.dimension_projection, 64)
			self.bn21 = nn.BatchNorm1d(64)
			self.fc20 = nn.Linear(64, 128)
			...
			self.fc7 = nn.Linear(768, 640)
			self.bn7 = nn.BatchNorm1d(640)
			self.fc8 = nn.Linear(640, 512)
			self.bn8 = nn.BatchNorm1d(512)
			self.fc9 = nn.Linear(512, 384)
			self.bn9 = nn.BatchNorm1d(384)
			self.fc10 = nn.Linear(384, 256)
			self.bn10 = nn.BatchNorm1d(256)
			self.fc11 = nn.Linear(256, 192)
			self.bn11 = nn.BatchNorm1d(192)
			self.fc12 = nn.Linear(192, 128)
			self.bn12 = nn.BatchNorm1d(128)
			self.fc13 = nn.Linear(128, 2*self.projection.dimension_projection)
			self.bn13 = nn.BatchNorm1d(2*self.projection.dimension_projection)
			self.fc14 = nn.Linear(2*self.projection.dimension_projection, 10)
			self.bn14  = nn.BatchNorm1d(10)
			self.fc15 = nn.Linear(10, 6)
			self.bn15 = nn.BatchNorm1d(6)
			self.fc16 = nn.Linear(6, self.nb_params)
	...
	def forward(self, s):
		...
		s64 = self.fc21(proj)
		s128 = self.fc20(F.relu(self.bn21(s64)))
